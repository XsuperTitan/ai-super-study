...
推荐方案：**腾讯云 CloudBase 云托管/云函数 + Node.js TypeScript 服务**。

推荐原因：

- 与微信生态天然靠近，便于后续获取 openid、小程序码、云存储、云数据库、消息推送等能力。
- 国内部署更适合小程序备案、数据合规和网络稳定性。
- Node.js/TypeScript 与小程序端语言栈一致，独立开发者维护成本低。
- 云托管适合承载 AI 请求、流式响应、内容解析等长耗时任务；云函数适合轻量事件和管理接口。
...
| 内容解析模块 | 将用户输入统一转为可出题文本 | MVP 只做文本清洗 |
...
- 后端服务：腾讯云 CloudBase 云托管。
- 临时缓存：CloudBase 云数据库或 Redis 兼容服务。
- 对象存储：CloudBase 云存储或腾讯云 COS。
...
interface ModelAdapter {
  generateQuiz(input: GenerateQuizModelInput): Promise<Quiz>;
  generateReport(input: GenerateReportModelInput): Promise<Report>;
  generateQuizStream?(
    input: GenerateQuizModelInput,
...
本项目第一阶段应避免“功能全、系统重、依赖多”的路线，优先用微信原生小程序和轻量 Node.js 后端跑通核心闭环。只要用户能把一段知识内容快速变成题目，并在答题后获得有价值的分析报告，产品就已经具备验证价值。

技术路线最终建议：

- 小程序端：微信原生小程序 + TypeScript。
- 后端：腾讯云 CloudBase 云托管/云函数 + Node.js TypeScript。
- AI：国内模型优先，统一 Model Adapter，强制结构化输出。
