...
> 更新日期：2026-06-04
> 当前策略：先把 M0 + 服务端历史记录打磨成稳定可自测、可演示、可真机验证的版本，再继续围绕 M1 做体验增强；不要立刻扩张到完整 M2 多源解析，也不要提前进入 M3 登录、支付和商业化系统。

### 15.1 当前结论

下一步开发章程是：**不要立刻做 M2 多源解析，也不要碰 M3 登录/商业化；先把 M0 + 服务端历史记录打磨成可稳定自测、可演示、可真机验证的版本，然后进入 M1 的流式/轮询/分享体验增强。**

当前项目已经具备：

- M0 核心闭环：输入 -> 生成题目 -> 答题 -> 报告。
- 后端 FastAPI + DeepSeek/Mock Adapter。
- 小程序页面闭环：输入页、生成页、答题页、报告页、历史页。
- MySQL 服务端历史记录前置能力。
- M1 阶段式流式生成、任务轮询降级和分享体验基础。
- M2.1 普通网页 URL 解析能力，已支持 `sourceType=url` 解析正文后出题。

因此近期重点不是继续堆功能，而是围绕 **稳定、验收、体验增强** 收口。
...
- 分别生成 3 题、5 题。
- 答完后进入报告页。
- 确认 MySQL `study_history_records` 有新增记录。
- 确认后端 `/health` 返回 `historyDatabase: true`。
- 确认 DeepSeek 失败时 Mock 兜底逻辑符合预期。

验收标准：

```text
输入内容 -> 生成题目 -> 答完 -> 生成报告 -> 写入 MySQL -> 历史页可回看
```

这一阶段先不新增大功能，只修阻塞闭环的问题。
...
1. **轮询降级**
   优先保证 `GET /api/v1/jobs/{jobId}` 稳定可用，因为它比流式更稳，也更适合真机兜底。

2. **流式响应**
   再接 `wx.request({ enableChunked: true })` + `onChunkReceived`。流式只是体验增强，不能作为唯一主路径。

3. **分享能力**
   完善 `onShareAppMessage`，分享先回首页或报告摘要页，不急着做海报。
...
原因：核心产品价值仍处于验证期。当前最重要的是证明用户愿意粘贴内容、完成一次闯关，并愿意回看报告或再次使用。
...
这一步跑稳后，再继续打磨轮询降级、流式响应和分享体验。

### 15.6 当前已完成开发

后端已完成：

- `POST /api/v1/quiz/generate`：普通出题接口，支持 DeepSeek 与 Mock 兜底。
...
- MySQL 历史记录：SQLAlchemy 2.x + PyMySQL + Alembic，使用 `DATABASE_URL` 控制是否启用。
- 历史接口：创建、列表、详情、删除单条、清空当前匿名用户历史。
- 普通网页 URL 解析：对 `sourceType=url` 做网页正文抽取，再进入出题流程。
- `sourceTrace` 误杀修复：URL/长文本不再因为模型返回片段未精确命中原文而 fallback Mock；空或不匹配时修复为正文前 100 字。

小程序端已完成：

- 首页输入、推荐知识 4 行 2 列网格、题量选择、启动生成。
- 首页操作区：启动知识跃迁、查看历史记录、继续答题进度。
- 自动识别普通网页 URL，向后端传递 `sourceType=url`。
- 生成页：优先流式，3 秒无 chunk、解析失败或连接中断时自动切换轮询，必要时再走普通接口兜底。
- 答题页：本地判题、解析展示、继续答题进度恢复。
- 报告页：生成报告、缓存报告、保存学习历史。
- 历史页：服务端优先、本地兜底，从历史打开报告不重新生成。
