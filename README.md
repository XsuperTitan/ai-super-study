...
- M2.3 B 站字幕解析：支持公开视频已有字幕提取并生成题目；无可解析字幕时尝试用视频标题和简介兜底，不做音频转写。
...
4. 从首页输入短主题、长文本、普通网页 URL、公开可访问的公众号文章链接或有字幕的 B 站公开视频链接。
5. 完成生成、答题、报告、历史回看流程。
...
```text
带有公开字幕的 B 站视频链接，例如 https://www.bilibili.com/video/...
```
...
- M2.3：B 站字幕可行性闭环已完成，字幕优先；无字幕 URL 时可用视频标题和简介兜底，不下载视频或音频。
- M2 后续：PDF/Doc、解析体验增强、OCR 继续按单一目标小步验证。
